...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('extension.blktool.showResultFile', () => {
    ...
    if (filename) {
      var extDir = vscode.extensions.getExtension("eguskov.blktool").extensionPath;
      child_process.execFile('blk.exe', ['-blk:' + document.fileName], { cwd: extDir }, function (err, data) {
        data = data.replace(/^[\r\n]/gm, '');
